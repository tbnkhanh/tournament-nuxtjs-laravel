    ...
    <div v-if="open" class="fixed z-10 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white w-1/2 p-6 rounded shadow-md" style="width: 500px;">
                ...
                <h2 class="text-2xl font-bold mb-4" style="text-align: center;">Edit Team</h2>
                ...
                <div v-for="(player, key) in formData.players" :key="player.id">
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Player {{ key+1 }} Name In 
                            Game</label>
                        <input type="text" id="name" name="player_in_game" v-model="player.in_game_name"
                            class="w-full p-2 border rounded-md focus:outline-none focus:border-blue-500">
                    </div>
                </div>

                <div style="text-align: center;">
                    <button type="submit" @click="handleSave(close)"
                        class="bg-blue-500 text-white font-bold py-2 px-4 rounded hover:bg-blue-700">Save Team</button>
                </div>
...
const handleSave = async (close) => {
    ...
    if (!formData.value.team_name.trim() || !formData.value.seed ) {
        validate = false;
    } else {
