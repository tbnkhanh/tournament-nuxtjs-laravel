    ...
    <div v-if="open" class="fixed z-10 inset-0 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen">
            <div class="bg-white w-1/2 p-6 rounded shadow-md" style="width: 500px; text-align: center;">
                ...
                <h2 class="text-2xl font-bold mb-4">Delete Team</h2>
                <p>Are you confirm delete team: {{ dataDelete.name }} ?</p>
                <div>
                    <button @click="handleDelete(dataDelete, close)"
...
async function handleDelete(dataDelete, close) {
    try {
        await useApiFetch("/sanctum/csrf-cookie");
        const { status, data } = await useApiFetch(`/api/team/delete/${dataDelete.teamId}`, {
            method: "DELETE"
        });
        if (status.value === 'success') {
            notify(data.value?.message)
            await useApiFetch(`/api/team/getTeamsWithPlayers/${dataDelete.tournamentId}`);
            close();
        }
